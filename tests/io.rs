...
macro_rules! test_file_no_cover {
    ( $function:ident, $file:expr ) => {
        #[test]
        fn $function() {
            let path = Path::new($file);
            ...
            tags.remove_track_number();
            assert!(tags.track_number().is_none());
            tags.remove_track_number();

            tags.set_total_tracks(10);
            assert_eq!(tags.total_tracks(), Some(10));
            tags.remove_total_tracks();
            assert!(tags.total_tracks().is_none());
            tags.remove_total_tracks();

            tags.set_disc_number(1);
            assert_eq!(tags.disc_number(), Some(1));
            tags.remove_disc_number();
            assert!(tags.disc_number().is_none());
            tags.remove_disc_number();

            tags.set_total_discs(2);
            assert_eq!(tags.total_discs(), Some(2));
            tags.remove_total_discs();
            assert!(tags.total_discs().is_none());
            tags.remove_total_discs();
        }
    };
}
...
test_file_no_cover!(test_ogg, "assets/a.ogg");
